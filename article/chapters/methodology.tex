...
This chapter describes the research methodology employed to investigate how AI Agent Protocols can enable semantic interoperability between heterogeneous Digital Twin systems.
The research follows a Design Science Research (DSR) approach, focusing on the design, development, and evaluation of a novel Digital Ecosystem architecture that leverages agent-mediated communication to solve cross-domain coordination challenges.

\section{Research Design}
\label{sec:research-design}

This research adopts the Design Science Research methodology proposed by Peffers et al.~\cite{peffers2007}, which provides a systematic framework for developing and evaluating Information Technology (IT) artifacts that address identified organizational problems.
DSR is particularly appropriate for this work because it emphasizes the creation and evaluation of purposeful artifacts---in this case, a Digital Ecosystem architecture and its proof-of-concept implementation.

The DSR process consists of six iterative activities:

\begin{enumerate}
    \item \textbf{Problem Identification and Motivation:} Addressed in Chapters~\ref{chap:introduction} and~\ref{chap:related-work}, identifying the semantic interoperability challenge in heterogeneous DT systems.
    
    \item \textbf{Objectives of a Solution:} Defined in Chapter~\ref{chap:objectives}, focusing on agent-mediated interoperability while preserving system autonomy.
    
    \item \textbf{Design and Development:} Described in Sections~\ref{sec:architecture-design} and~\ref{sec:implementation}, covering the Digital Ecosystem architecture and agent protocol integration.
    
    \item \textbf{Demonstration:} Presented in Section~\ref{sec:use-case}, using a realistic cross-domain scenario involving Smart City and Energy Grid DTs.
    
    \item \textbf{Evaluation:} Detailed in Section~\ref{sec:evaluation}, with metrics assessing semantic mapping capabilities, integration effort, and functional success.
    
    \item \textbf{Communication:} Fulfilled through this thesis and planned publications.
\end{enumerate}
...
The proposed Digital Ecosystem architecture draws inspiration from the Destination Earth (DestinE) initiative's virtual cloud layer~\cite{Nativi_Craglia_2021}, adapting its principles for agent-mediated DT interoperability.
...
Following DestinE's architectural principles, the virtual cloud layer virtualizes heterogeneous operational infrastructure environments while providing unified access through standardized interfaces.
For this research, a minimalist centralized orchestration approach has been selected to reduce implementation complexity while maintaining core capabilities for agent-mediated interoperability.

The virtual cloud architecture comprises:

\begin{itemize}
    \item \textbf{DT Platform Layer:} Independent KTWIN and Eclipse Ditto clusters, each maintaining domain-specific optimizations and ontologies.
    
    \item \textbf{Mediation Layer:} Agent-based services that broker semantic interactions between heterogeneous platforms using MCP for resource discovery and A2A for negotiation.
    
    \item \textbf{Orchestration Layer:} Centralized coordination services managing cross-domain requests and routing agent communications.
    
    \item \textbf{Service API Layer:} Common interfaces for external clients to submit cross-domain queries without platform-specific knowledge.
\end{itemize}

This layered approach isolates platform heterogeneity from cross-domain coordination logic, enabling agents to focus on semantic mediation rather than protocol translation.
...
The deliberate selection of platforms with similar messaging infrastructure (RabbitMQ) but different knowledge representations (DTDL vs. Thing model) creates realistic heterogeneity while maintaining implementation feasibility.
...
The SmartCity Agent (paired with KTWIN) specializes in DTDL-based smart city ontologies, while the EnergyGrid Agent (paired with Ditto) specializes in CIM-based grid ontologies translated to the Thing model.
...
A lightweight orchestration agent coordinates multi-platform interactions, routing requests to appropriate platform agents and aggregating responses.
This centralized coordination simplifies the proof-of-concept while maintaining the agent autonomy principle---platform agents retain full control over their semantic mappings and never directly access other platforms.

\section{Cross-Domain Use Case Definition}
\label{sec:use-case}

To validate the agent-mediated interoperability approach, a realistic cross-domain scenario has been designed that requires cooperation between Smart City and Energy Grid DT systems.

\subsection{Scenario Description: Dynamic EV Charging Coordination}
\label{subsec:scenario-description}

The use case addresses a practical urban energy management challenge: coordinating electric vehicle (EV) charging to prevent distribution grid overloads while maximizing charging service availability.
...
The scenario requires bridging multiple semantic gaps between the \texttt{opendigitaltwins-smartcities} ontology (KTWIN) and the CIM-based \texttt{opendigitaltwins-energygrid} ontology (Ditto):
...
\hline
\texttt{ChargingStation} (location entity) & \texttt{UsagePoint} (metering point) & Different abstraction levels: physical location vs. consumption measurement point \\
\hline
\texttt{chargingCapacity} (kW available) & \texttt{EquivalentLoad.activePower} (kW consumed) & Opposite perspectives: supply capacity vs. demand consumption \\
\hline
Geographic location (lat/lon) & Grid topology (substation/feeder hierarchy) & Spatial vs. topological representation requiring mapping \\
\hline
\texttt{hasChargingCapability} (boolean) & \texttt{UsagePoint} linked to \texttt{Transformer} & Binary property vs. relational topology \\
\hline
\texttt{ParkingSpot.occupancy} & Load forecasting window & Temporal granularity mismatch \\
\hline
\end{tabular}
...
These gaps are representative of real-world DT integration challenges: different domains model the same physical infrastructure from different perspectives using domain-specific ontologies optimized for their use cases.
...
The implementation follows an iterative agent-first development process, prioritizing early validation of the core agent protocol capabilities before full system integration.

\subsection{Development Phases}
\label{subsec:development-phases}

\textbf{Phase 1: Agent Protocol Spike (Weeks 1-2)}
...
\textbf{Phase 2: Single DT Platform Integration (Weeks 3-4)}
...
\textbf{Phase 3: Heterogeneous Platform Integration (Weeks 5-7)}
...
\textit{Deliverable:} Agents successfully negotiate \texttt{ChargingStation} $\leftrightarrow$ \texttt{UsagePoint} mapping and answer cross-domain query: ``Which charging stations connect to Transformer T1?''

\textbf{Phase 4: Complete Use Case and Orchestration (Weeks 8-10)}
...
Time-series event replay enables deterministic scenario execution for controlled evaluation while introducing configurable variability to test agent adaptability.

\section{Evaluation Methodology}
\label{sec:evaluation}

The evaluation assesses whether agent-mediated interoperability achieves the claimed benefits over traditional integration approaches across three dimensions: semantic capability, integration effort, and functional performance.

\subsection{Evaluation Criteria}
\label{subsec:evaluation-criteria}

\textbf{Research Question:} How can AI Agent Protocols improve Digital Twin systems interoperability while preserving existing system autonomy?

This question is operationalized through three evaluation criteria:

\begin{enumerate}
    \item \textbf{Semantic Interoperability:} Can agents dynamically establish semantic mappings between heterogeneous ontologies without pre-coordination?
    
    \item \textbf{Integration Effort:} Does the agent approach reduce implementation and maintenance burden compared to traditional integration?
    
    \item \textbf{Functional Effectiveness:} Does agent-mediated coordination successfully solve the cross-domain use case?
\end{enumerate}

\subsection{Metrics Definition}
\label{subsec:metrics-definition}
...
\subsection{Research Scope Boundaries}
\label{subsec:scope-boundaries}

This research explicitly excludes several important aspects of production DT systems:

\textbf{Not Addressed:}
\begin{itemize}
    \item \textbf{Real-time guarantees:} No hard latency bounds or deterministic timing
    \item \textbf{Security and authentication:} No encryption, access control, or authentication mechanisms
    \item \textbf{Production deployment:} No high availability, disaster recovery, or operational monitoring
    \item \textbf{Proprietary platform integration:} Focus limited to open-source platforms
    \item \textbf{Legal and regulatory compliance:} No analysis of data governance or GDPR implications
\end{itemize}

\subsection{Simulation Limitations}
\label{subsec:simulation-limitations}

The simulation-based evaluation has inherent limitations:

\begin{itemize}
    \item \textbf{Simplified physics:} EV charging and grid load models are approximations of complex electrical systems
    \item \textbf{Synthetic data:} Real-world operational data may exhibit patterns not captured in simulation
    \item \textbf{Scale constraints:} Limited to 50 EVs and 3 transformers; larger-scale behavior unknown
    \item \textbf{Controlled environment:} Lacks unpredictability of production systems (sensor failures, network issues, adversarial inputs)
\end{itemize}

\subsection{Generalizability Considerations}
\label{subsec:generalizability}

Results should be interpreted within context:

\begin{itemize}
    \item \textbf{Domain specificity:} Findings apply to Smart City $\leftrightarrow$ Energy Grid integration; other domain pairs may have different characteristics
    \item \textbf{Ontology dependency:} Success depends on DTDL expressiveness; other knowledge representation formalisms may behave differently
    \item \textbf{Agent reasoning:} LLM-based semantic reasoning quality depends on model capabilities and prompt engineering
    \item \textbf{Platform selection:} KTWIN and Ditto share architectural similarities (event-driven, RabbitMQ); more divergent platforms may pose additional challenges
\end{itemize}

\subsection{Ethical Considerations}
\label{subsec:ethical-considerations}

While this research focuses on technical interoperability, several ethical dimensions merit acknowledgment:

\begin{itemize}
    \item \textbf{Energy access equity:} EV charging optimization algorithms should not systematically disadvantage certain user groups
    \item \textbf{Data privacy:} Production implementations would require careful handling of location and consumption data
    \item \textbf{Transparency:} Agent-mediated decision-making should be explainable to system operators
    \item \textbf{Environmental impact:} Computational overhead of agent reasoning should be considered in sustainability analysis
\end{itemize}

These considerations are outside the scope of this technical proof-of-concept but would require attention in real-world deployments.
...
\begin{itemize}
    \item Agent implementation quality may affect results independent of protocol design
    \item Performance measurements may be influenced by hardware/network conditions
    \item Mitigation: Multiple experimental runs, controlled environment, code review
\end{itemize}

\textbf{External Validity:}
\begin{itemize}
    \item Use case may not represent full spectrum of DT interoperability challenges
    \item Open-source platforms may differ significantly from proprietary industrial systems
    \item Mitigation: Clear scope definition, comprehensive related work comparison
\end{itemize}

\textbf{Construct Validity:}
\begin{itemize}
    \item Metrics may not fully capture ``semantic interoperability'' or ``system autonomy''
    \item Mitigation: Multiple complementary metrics, qualitative assessment
\end{itemize}

Despite these limitations, the research provides valuable insights into the feasibility and potential benefits of agent-mediated DT interoperability, establishing a foundation for future work addressing production deployment challenges.