...
\begin{figure}[htbp]
    \centering
    \includegraphics[height=0.85\textheight,keepaspectratio]{figures/architecture.png}
    \caption{
...
The session layer manages multi-agent conversations that coordinate
    cross-domain interactions between heterogeneous DT platforms and user agents.
This layer establishes which agents participate in solving a given request,
    defines the coordination pattern
    (e.g., sequential queries, parallel aggregation, or negotiation),
    and enforces interaction protocols that all agents must follow.

The \textbf{Agent Gateway} component realizes these principles through
    three core mechanisms.
First, it exposes the A2A protocol interface to user agents,
    providing a single point of entry for cross-domain requests
    without requiring platform-specific knowledge.
Second, it discovers available domain agents and their capabilities,
    enabling dynamic planning of interactions based on available resources.
When a user request arrives, the Agent Gateway queries domain agents to understand
    their capabilities, then decomposes the user's goal
    into a coordinated sequence of agent interactions.
Third, it facilitates conversations between platform-specific agents,
    coordinating their semantic negotiations and information exchanges
    to fulfill complex user goals.

Resources are exclusively accessed through domain layer agents,
    which encapsulate platform-specific logic and knowledge.
The Agent Gateway never directly interacts with DT platforms or their data sources;
    instead, it communicates exclusively via the A2A protocol with domain agents,
...
Policy enforcement occurs at the gateway boundary, enabling centralized control
